...
// The two functions below are adapted from
// https://stackoverflow.com/questions/5678932/fastest-way-to-read-numerical-values-from-text-file-in-c-double-in-this-case/5679087#5679087
// many thanks to @Potatoswatter and @ildjarn
...
void read_entry(size_t i, size_t j, double* dest) {
    std::string fname = "data/MM_" + std::to_string(i + 1) + std::to_string(j + 1) + ".dat";
    std::FILE* infile = std::fopen(fname.c_str(), "r");
    if (infile == NULL) {
        std::cerr << "File " << fname << " does not exist" << std::endl;
        return;
    }
    const size_t buf_size = WIDTH * (MAX_PRECISION + 1);
    char buffer[buf_size];
    size_t counter = i * DIM + j;
    const size_t step = DIM * DIM;

    while (std::fgets(buffer, buf_size, infile) != NULL) {
        char const* cursor = buffer;
        for (size_t k = 0; k < WIDTH; ++k) {
            dest[counter] = mystrtof(cursor);
            counter += step;
        }
    }
    std::fclose(infile);
...
double* read_matrices() {
    size_t total_length = WIDTH * HEIGHT * DIM * DIM;
